...
export const metadata: Metadata = {
  ...
  description: "Follow Malone's Pub pool tournament, view the bracket and match results, and read the official rules.",
  alternates: { canonical: "https://malonespub.com/pool-tournament" },
  openGraph: {
    title: "Malone's Pool Tournament | Bracket & Rules",
    description: "Pool tournament matchups, results, and rules for Malone's Pub in Downtown Fort Worth.",
...
export default async function PoolTournamentPage() {
  ...
  const accepting = !tournament.bracket && tournament.registrationStatus === "Open" && tournament.availableSpots > 0;
  const facts = tournament.bracket
    ? [["Status", tournament.bracket.champion ? "Completed" : "Registration full"], ["Players", "16 players"], ["Entry fee", tournament.entryFee || "To be announced"], ["Matches", "Best of 3 · 8-ball"]]
    : [["Date", date], ["Check-in", checkIn], ["Start time", start], ["Entry fee", tournament.entryFee || "To be announced"], ["Player limit", `${tournament.maxPlayers} players`], ["Available", `${tournament.availableSpots} ${tournament.availableSpots === 1 ? "spot" : "spots"}`]];
  ...
  return (
    <>
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className={`relative mx-auto grid max-w-6xl items-center gap-8 px-4 lg:grid-cols-[1.15fr_.85fr] ${tournament.bracket ? "py-10" : "min-h-[500px] py-14"}`}>
          <div>
            ...
            <p className="mt-5 max-w-xl text-lg leading-8 text-neutral-200">{tournament.bracket ? "The tournament is full. Follow the matchups and results below." : accepting ? "Register from your phone, then arrive by check-in time ready to play." : "Registration is closed. Check back here for tournament matchups."}</p>
            <div className="mt-7 flex flex-wrap gap-3">
              <a href={tournament.bracket ? "#bracket" : "#register"} className="rounded-lg bg-green-500 px-5 py-3 font-black text-neutral-950">{tournament.bracket ? "View bracket" : accepting ? "Register now" : "Registration closed"}</a>
          ...
          <div className="grid grid-cols-2 gap-3 rounded-2xl border border-white/15 bg-black/65 p-4 backdrop-blur sm:p-6">
            {facts.map(([label, value]) => <div key={label} className="rounded-lg bg-white/5 p-3"><p className="text-xs font-bold uppercase tracking-wider text-neutral-400">{label}</p><p className="mt-1 font-black text-white">{value}</p></div>)}
          </div>
        </div>
      </section>

      {tournament.bracket ? <PoolTournamentBracket bracket={tournament.bracket} allowPublicScoreEntry={!tournament.bracket.champion} /> : null}

      <section className={`mx-auto grid max-w-6xl gap-8 px-4 py-14 ${tournament.bracket ? "" : "lg:grid-cols-[1fr_420px]"}`}>
        <div className="space-y-8">
          ...
          <details id="rules" className="scroll-mt-24 rounded-xl border border-white/10 bg-neutral-900 p-5">
        ...
        {!tournament.bracket ? <div id="register" className="scroll-mt-24"><RegistrationForm acceptingRegistrations={accepting} dateLabel={date} startTimeLabel={start} /></div> : null}
      </section>
    </>
  );
}
